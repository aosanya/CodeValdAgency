...
// ArangoDB.
//
// Entity collections are derived entirely from TypeDefinition.StorageCollection
// in the schema passed to New / NewBackend / NewBackendFromDB. No collection
// names are hardcoded in this package — adding or renaming a type in schema.go
// is the only change required.
//
// Infrastructure collections that are not schema-driven:
//   - agency_entities           — fallback for TypeIDs without a StorageCollection
//   - agency_relationships      — directed graph edges (ArangoDB edge collection)
//   - agency_schemas_draft      — mutable draft schema documents
//   - agency_schemas_published  — immutable published schema snapshots
//
// File layout:
